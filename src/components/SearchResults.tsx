...
export function SearchResults() {
  ...
  useEffect(() => {
    const handleSearch = async (formData: FormData) => {
      ...
      try {
        const results = await fetchSearchResults(formData);
        setState({ data: results, error: null, isLoading: false });
      } catch (error) {
        setState({
          data: [],
          error:
  ...
  const handleViewDetails = (result: SearchResult) => {
    ...
    router.push(`/details/${result.rollNo}?${params.toString()}`);
  };
